...
<body>
    ...
    <div class="banner">
        <ul>
            ...
            <a href="question_view.php"><li class="menu_active">질문 게시판</li></a>
            <a href="free_view.php"><li class="menu">자유 게시판</li></a>
        </ul>
    </div>
    <table class=middle>
        <thead>
        ...
        <?php
            ...
            $sql = "SELECT * FROM question_borde ORDER BY qid DESC limit $start, 10";
            $res = mysqli_query($conn, $sql);

            while($row = mysqli_fetch_array($res)){ ?>
            <tbody>
                <tr class="post-row" data-post-id="<?php echo $row['qid']; ?>">
    ...
    <?php
        // 페이징 링크 생성
        $sql = "SELECT COUNT(*) AS qid FROM question_borde";
        $res = mysqli_query($conn, $sql);
...
$(document).ready(function() {
    $(".post-row").click(function() {
        ...
        // 조회수를 증가시키는 페이지로 이동
        window.location.href = '../PHP/hits.php?qid=' + postId;
    });
});
</script>