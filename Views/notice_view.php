...
<body>
    ...
    <div class="banner">
        <ul>
            <a href="notice_view.php"><li class="menu_active">공지 게시판</li></a>
    ...
    <table class=middle>
        ...
        <?php
            ...
            $sql = "SELECT * FROM nootice_borde ORDER BY nid DESC limit $start, 10";
            $res = mysqli_query($conn, $sql);


            while($row = mysqli_fetch_array($res)){ ?>
            <tbody>
                <tr class="post-row" data-post-id="<?php echo $row['nid']; ?>">
    ...
    <?php
        // 페이징 링크 생성
        $sql = "SELECT COUNT(*) AS nid FROM nootice_borde";
        $res = mysqli_query($conn, $sql);
...
<script>
  ...
  $(document).ready(function() {
    $(".post-row").click(function() {
        ...
        // 조회수를 증가시키는 페이지로 이동
        window.location.href = '../PHP/hits.php?nid=' + postId;
    });
});
</script>